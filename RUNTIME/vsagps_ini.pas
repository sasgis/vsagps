...
const
  vsagps_ini_filename = 'vsagps_nmea.ini';
...
procedure VSAGPS_ini_LoadFromFile(sl: TStrings);
...
var fs: TFileStream;
begin
  if (not FileExists(vsagps_ini_filename)) then
    Exit;
  fs := TFileStream.Create(vsagps_ini_filename, fmOpenRead);
  try
...
function VSAGPS_ini_GetSectionHeader(sl_Ini: TStrings; const SectionNameUppercased: String): Integer;
var s: String;
begin
  Result:=0;
  while Result<sl_Ini.Count do begin
    s:=sl_Ini[Result];
    if (0<Length(s)) then
    if (vsagps_ini_sectiona=s[1]) then begin
      // start of some section
      s:=VSAGPS_ini_DelComment(s);
      if SameText(s, vsagps_ini_sectiona+SectionNameUppercased+vsagps_ini_sectionb) then
        Exit;
    end;
    Inc(Result);
  end;
...
begin
  Result:=FALSE;
  i:=VSAGPS_ini_GetSectionHeader(sl_Ini, SectionNameUppercased);
  if (i>=0) then begin
    Inc(i);
